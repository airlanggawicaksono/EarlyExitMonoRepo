...
def streamer(mocker):
    ...
    tokenizer.decode = mocker.MagicMock(
        side_effect=lambda tokens, **kwargs: " ".join(str(x) for x in tokens)
    )
...
def test_put_tokens_blocking(streamer):
    ...
    streamer.tokenizer.decode.assert_called_once_with(
        [1, 2, 3], **streamer.decode_kwargs
    )
