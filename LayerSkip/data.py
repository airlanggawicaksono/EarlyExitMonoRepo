...
def get_valid_dataset_formats():
    ...
    return [
        value
        for key, value in DatasetFormat.__dict__.items()
...
def prepare_evaluation_examples_chat_format(
    data_path: str, template: str = None
        ...
        while i < len(json_line["data"]):
            if json_line["data"][i]["role"] == "PARSER":
                prompt = apply_template(
                    message=stringify_conversation(json_line["data"][1:i])
                    + "\n[PARSER]\n",
                    template=template,
                )
...
def prepare_cnn_dm_lm_format(template: str = None) -> List[EvaluationExample]:
    ...
    for data_point in load_dataset("cnn_dailymail", "3.0.0")["test"]:
        ...
        prompt = apply_template(
            message=" ".join(words[:PREFIX_LENGTH]), template=template
        )
        evaluation_data_points.append(
            EvaluationExample(
                input=prompt,
...
def prepare_cnn_dm_summarization_format(
    n_shot: int = 0, seed: int = 42, template: str = None
) -> List[EvaluationExample]:
    prompt_shots = ""
    if n_shot > 0:
        prompt_keys = ["article", "highlights"]
        shots = (
            load_dataset("cnn_dailymail", name="3.0.0", split="train")
            .shuffle(seed=seed)
            .select(range(n_shot))
        )
...
def prepare_xsum_summarization_format(
    ...
    if n_shot > 0:
        ...
        shots = (
            load_dataset("xsum", split="train").shuffle(seed=seed).select(range(n_shot))
        )
        for i in range(n_shot):
            prompt = (
                "Article: "
...
def get_data(
    ...
    if dataset == DatasetFormat.CHAT_FORMAT:
        evaluation_data_points = prepare_evaluation_examples_chat_format(
            data_path, template=template
        )
    elif dataset == DatasetFormat.CNN_DM_SUMMARIZATION:
        evaluation_data_points = prepare_cnn_dm_summarization_format(
            n_shot=n_shot, seed=seed, template=template
        )
    elif dataset == DatasetFormat.XSUM_SUMMARIZATION:
        evaluation_data_points = prepare_xsum_summarization_format(
            n_shot=n_shot, seed=seed, template=template
        )
        ...
        evaluation_data_points = prepare_custom(
            data_path,
