...
def _load_model(model_id: str, strategy: str, num_labels: int, compile_model: bool = False):
    ...
    if strategy == "entropy":
        from models.modeling_elasticbert_entropy import ElasticBertForSequenceClassification  # type: ignore
    else:
        from models.modeling_elasticbert_patience import ElasticBertForSequenceClassification  # type: ignore
...
def _load_loader(model_id: str, task: str, data_dir: Union[str, Path], out_dir: Path, max_seq_length: int):
    ...
    eval_args = argparse.Namespace(
        ...
        overwrite_cache=False, model_type="elasticbert",
    )
    eval_dataset = load_and_cache_examples_glue(eval_args, eval_args.task_name, tokenizer, data_type='dev')
    return tokenizer, DataLoader(eval_dataset, sampler=SequentialSampler(eval_dataset), batch_size=1)
...
# =============================================================================
def profile_hw(
    model_id: str, task: str, strategy: str, threshold: Union[int, float],
    data_dir: Union[str, Path], out_dir: Union[str, Path],
    *, max_seq_length: int = 128, warmup_steps: int = 3,
    ...
    with BenchmarkProfiler(
        out_path=out_path, task=task, strategy=strategy,
        threshold=threshold, warmup_steps=warmup_steps,
    ) as prof:
...
def evaluate_quality(
    ...
    *, max_seq_length: int = 128,
    ...
    metrics = glue_compute_metrics(task.lower(),
                                   torch.tensor(preds).numpy(),
                                   torch.tensor(labels).numpy())
    out_dir.mkdir(parents=True, exist_ok=True)
    out_path.write_text(json.dumps({
        "task": task, "strategy": strategy, "threshold": threshold,
        "n_samples": len(preds), **metrics,
...
def benchmark(
    model_id: str, task: str, strategy: str, threshold: Union[int, float],
    data_dir: Union[str, Path], out_dir: Union[str, Path],
    *, max_seq_length: int = 128, bench_batch: int = 1, warmup_steps: int = 3,
    use_torch_compile: bool = True,
) -> Tuple[Path, Path]:
    hw = profile_hw(model_id, task, strategy, threshold, data_dir, out_dir,
                    max_seq_length=max_seq_length, warmup_steps=warmup_steps,
                    use_torch_compile=use_torch_compile)
    q  = evaluate_quality(model_id, task, strategy, threshold, data_dir, out_dir,
                          max_seq_length=max_seq_length)
    return hw, q
