...
def train(
    ...
    *,
    ...
    if skip_if_exists:
        ...
        try:
            ...
            if any(f.endswith(".bin") or f.endswith(".safetensors") for f in files):
                print(
                    f"[train] HF checkpoint already exists: {repo}, skipping training"
                )
                return out_dir
        except Exception:
    ...
    cmd = [
        ...
        str(C.GLUE_DIR / task),
        "--log_dir",
        str(C.LOG_DIR / "elasticbert-base/glue" / task),
        "--output_dir",
        str(out_dir),
        "--num_hidden_layers",
        str(C.NUM_HIDDEN_LAYERS),
        "--num_output_layers",
        str(C.NUM_OUTPUT_LAYERS),
        "--max_seq_length",
