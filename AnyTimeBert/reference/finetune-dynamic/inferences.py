...
def inference_elue_patience(args, model, tokenizer, prefix="", patience=0):
    ...
    output_infer_file = os.path.join(
        eval_output_dir, "{}_{}.tsv".format(eval_task, str(patience))
    )
    ...
    output_infer_file = os.path.join(
        eval_output_dir,
        "{}_{}_{}_{}.tsv".format(eval_task, "infer", "exit_layer", str(patience)),
    )
    with open(output_infer_file, "w", encoding="utf-8") as fout:
        writer = csv.writer(fout, delimiter="\t", quotechar=None)
...
def inference_elue_entropy(
    args, model, tokenizer, prefix="", eval_highway=False, entropy=0.0
):
    ...
    output_infer_file = os.path.join(
        eval_output_dir, "{}_{}.tsv".format(eval_task, str(entropy))
    )
    with open(output_infer_file, "w", encoding="utf-8") as fout:
        writer = csv.writer(fout, delimiter="\t", quotechar=None)
        writer.writerow(["index", "prediction"])
    ...
    output_infer_file = os.path.join(
        eval_output_dir,
        "{}_{}_{}_{}.tsv".format(eval_task, "infer", "exit_layer", str(entropy)),
    )
    with open(output_infer_file, "w", encoding="utf-8") as fout:
        writer = csv.writer(fout, delimiter="\t", quotechar=None)
        writer.writerow(["index", "exit_layer"])
