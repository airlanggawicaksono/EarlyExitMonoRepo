                ...
                if (
                    ...
                    if (
                        ...
                        for i, results in enumerate(results_all):
                            ...
                            fitlog.add_metric(
                                {"exit_" + str(i): res_for_display}, step=global_step
                            )
...
def main():
    ...
    train_dataset = None
    if args.do_train:
        train_dataset = load_and_cache_examples_elue(
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        for entropy in entropy_list:
            ...
            for checkpoint in checkpoints:
                ...
                result, speed_up, exit_layer_every_ins = evaluate_elue_entropy(
                    args,
                    model,
                    tokenizer,
                    prefix=prefix,
                ...
                for k, v in result.items():
                    res_for_display[k.replace("-", "_")] = v
                fitlog.add_metric(
                    {"dev": res_for_display, "speed_up": speed_up},
                ...
                if result[metric_key] > best:
                    ...
                    fitlog.add_best_metric(
                        {
                            "dev": {
                                metric_key.replace("-", "_"): result[metric_key],
                                "speed_up": speed_up,
                            }
                        }
                    )
                    ...
                    output_eval_file = os.path.join(
                        args.output_dir,
                        "{}_{}_{}_{}.tsv".format(
                            args.task_name, "eval", "exit_layer", str(entropy)
                        ),
                    )
                    ...
                    infer_speed_up = inference_elue_entropy(
                        args,
                        model,
                        tokenizer,
                        prefix=prefix,
                    ...
                    fitlog.add_metric(
                        {"test_speed_up": infer_speed_up}, step=int(global_step)
                    )
