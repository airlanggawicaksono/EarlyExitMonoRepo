...
class DualDSegment(DualDDetect):
    ...
    def forward(self, x):
        ...
        mc = [
            torch.cat(
                [self.cv6[i](x[i]).view(bs, self.nm, -1) for i in range(self.nl)], 2
            ),
            torch.cat(
                [
                    self.cv7[i](x[self.nl + i]).view(bs, self.nm, -1)
                    for i in range(self.nl)
                ],
                2,
            ),
...
class DetectionModel(BaseModel):
    ...
    def __init__(
        ...
        if isinstance(
            m, (DualDetect, TripleDetect, DualDDetect, TripleDDetect, DualDSegment)
        ):
            s = 256  # 2x min stride
            m.inplace = self.inplace
            forward = lambda x: (
