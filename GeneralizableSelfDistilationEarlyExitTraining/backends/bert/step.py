...
def cascade_step(model, stage, batch, cfg):
    """LoRAExit: all adapters at once. Deepest anchored on labels; exit i learns
    from exit i+1 (detached). One backward updates every adapter."""
    _, labels = _inputs(batch)
    n = model.n_exits
    ...
    for i in range(n - 1):
        loss = loss + distill_loss(
            logits[i], logits[i + 1].detach(), labels,
            temperature=cfg.temperature,
            alpha_kd=cfg.alpha_kd,
            use_true_labels=cfg.use_true_labels,
        )
