"""Vision orchestration: cfg -> model -> plan -> run each stage -> save. Thin
glue. No mode branches; per-stage setup + step + saver dispatched on stage
attributes via dicts. Reuses the shared plan.MODE_BUILDERS.
"""
...
def train(cfg):
    _set_seed(cfg.seed)
    num_labels = count_labels(cfg.dataset)
