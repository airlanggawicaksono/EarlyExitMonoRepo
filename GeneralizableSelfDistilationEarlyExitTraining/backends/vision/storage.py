"""Disk IO for ViT self-distill: adapters + per-exit head + metrics. Mirrors
BERT storage; layout per stage:

    <run_dir>/<stage.label>/
        adapter/                # peft adapter weights (lora stages)
        head_<exit>.pt          # per-exit linear head
        full_model.pt           # whole model (joint stage)
        metrics.json            # resume marker
"""
