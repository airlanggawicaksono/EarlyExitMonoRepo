...
def _load_msdnet(model_id: str, dataset: str, compile_model: bool = False):
    ...
    args = argparse.Namespace(
        ...
        grFactor=[
            int(x) for x in (("1-2-4-4" if is_imagenet else C.GR_FACTOR).split("-"))
        ],
        bnFactor=[
            int(x) for x in (("1-2-4-4" if is_imagenet else C.BN_FACTOR).split("-"))
        ],
        base=C.BASE,
    ...
    *,
    ...
    for inputs, targets in tqdm(loader, desc=f"Q  {dataset} {eval_mode}"):
        ...
        for i, logits in enumerate(outputs):
            ...
            top5 = sum(
                (targets[j] in logits[j].topk(5).indices)
